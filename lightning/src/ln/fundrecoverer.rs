...
use crate::chain::{self, channelmonitor};
...
use crate::chain::channelmonitor::{
	...
	WithChannelMonitor, STUB_CHANNEL_UPDATE_IDENTIFIER, read_util, ReadUtilOpt, get_stub_channel_info_from_ser_channel
};
...
use crate::sign::{EntropySource, NodeSigner, SignerProvider, KeysManager, InMemorySigner};
	...
	F::Target: FeeEstimator,
{
	...
	fn handle_your_peer_storage(
		...
		for ((node_id, channel_id), min_seen_secret) in our_peer_storage.get_cid_and_min_seen_secret().unwrap() {
			println!("here i am inside fundrecoverer {:?} {:?} {:?}", node_id, channel_id, min_seen_secret);
				...
				match get_stub_channel_info_from_ser_channel(&chan_bytes) {
					Ok(ps_channel) => {
						...
						if stale || missing {
							...
							println!("ps_channel channel id is {:?}", ps_channel.cid);
							let (blockhash, monitor) = read_util::<_, ChannelSigner, DummySignerProvider<ChannelSigner>, DummyEntropySource>(&mut ::bitcoin::io::Cursor::new(chan_bytes),
