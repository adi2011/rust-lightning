...
use crate::chain;
use crate::chain::{BestBlock, WatchedOutput, ClaimId};
...
pub fn get_stub_channel_info_from_ser_channel(ser_channel: &Vec<u8>) -> Result<StubChannelInfo, DecodeError> {
	let chan_reader = &mut ::bitcoin::io::Cursor::new(ser_channel);
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub(crate) fn new(secp_ctx: Secp256k1<secp256k1::All>, keys: Signer, shutdown_script: Option<ScriptBuf>,
		...
		let commitment_secrets = CounterpartyCommitmentSecrets::new();
	...
	// fn get_new_stub(self) -> ChannelMonitor<Signer> {
	// 	let mon = self.inner.lock().unwrap();
	// 	mon.onchain_tx_handler.
	// 	let dummy_key = PublicKey::from_slice(&[2;33]).unwrap();
	// 	let holder_commitment_tx = HolderSignedTx {
	// 		txid: mon.get_funding_txo().0.txid,
	// 		revocation_key: RevocationKey(dummy_key),
	// 		a_htlc_key: HtlcKey(dummy_key),
	// 		b_htlc_key: HtlcKey(dummy_key),
	// 		delayed_payment_key: DelayedPaymentKey(dummy_key),
	// 		per_commitment_point: dummy_key,
	// 		htlc_outputs: Vec::new(), // Not copying it directly to avoid storing HTLCs
	// 		to_self_value_sat: 0,
	// 		feerate_per_kw: 1,
	// 	};
	// 	let mut counterparty_claimable_outpoints = new_hash_map();
	// 	counterparty_claimable_outpoints.insert(mon.current_counterparty_commitment_txid.unwrap(), Vec::new());

	// 	let onchain_tx_handler = OnchainTxHandler::new(
	// 		mon.channel_value_satoshis, mon.channel_keys_id, mon.destination_script.clone(), mon.onchain_tx_handler.signer.clone(),
	// 		mon.onchain_tx_handler.channel_transaction_parameters.clone(), mon.onchain_tx_handler.get_holder_commitment(), Secp256k1::new()
	// 	);

	// 	ChannelMonitor::from_impl(ChannelMonitorImpl {
	// 		latest_update_id: STUB_CHANNEL_UPDATE_IDENTIFIER,
	// 		commitment_transaction_number_obscure_factor: mon.commitment_transaction_number_obscure_factor,
	// 		destination_script: mon.destination_script.clone(),
	// 		broadcasted_holder_revokable_script: None,
	// 		counterparty_payment_script: mon.counterparty_payment_script.clone(),
	// 		shutdown_script: None,
	// 		channel_keys_id: mon.channel_keys_id,
	// 		holder_revocation_basepoint: mon.holder_revocation_basepoint,
	// 		channel_id: mon.channel_id,
	// 		funding_info: (mon.funding_info.0, ScriptBuf::new()),
	// 		current_counterparty_commitment_txid: None,
	// 		prev_counterparty_commitment_txid: None,
	// 		counterparty_commitment_params: mon.counterparty_commitment_params.clone(),
	// 		funding_redeemscript: ScriptBuf::new(),
	// 		channel_value_satoshis: mon.channel_value_satoshis,
	// 		their_cur_per_commitment_points: mon.their_cur_per_commitment_points,
	// 		on_holder_tx_csv: mon.on_holder_tx_csv,
	// 		commitment_secrets: mon.commitment_secrets.clone(),
	// 		counterparty_claimable_outpoints,
	...
	// 		is_processing_pending_events: false,
	// 		onchain_events_awaiting_threshold_conf: Vec::new(),
	// 		outputs_to_watch: mon.outputs_to_watch.clone(),
	// 		onchain_tx_handler,
	// 		lockdown_from_offchain: true,
	// 		holder_tx_signed: true,
	// 		funding_spend_seen: false,
	...
	// 		htlcs_resolved_on_chain: Vec::new(),
	// 		spendable_txids_confirmed: Vec::new(),
	// 		best_block: mon.best_block,
	// 		counterparty_node_id: mon.counterparty_node_id,
	// 		initial_counterparty_commitment_info: None,
	// 		balances_empty_height: None
	// 	})
	// }
...
// struct onchaintx_builder_data {}
const MAX_ALLOC_SIZE: usize = 64*1024;
pub enum ReadUtilOpt<'a, 'b, ChannelSigner: EcdsaChannelSigner, SP: SignerProvider<EcdsaSigner = ChannelSigner>, ES: EntropySource>
{
	IsStub {keys: ChannelSigner, secp_ctx: Secp256k1<secp256k1::All>},
	NotStub {entropy_source: &'a ES, signer_provider: &'b SP}
}
	...
	ES: EntropySource,
{
	...
	// let mut destination_script: ScriptBuf = Readable::read(reader)?;
	...
	// let destination_script = ScriptBuf::from(buf);
	...
	// let mut counterparty_payment_script: ScriptBuf = ScriptBuf::new();
	let lenn = <u16 as Readable>::read(reader)? as usize;
	let mut buff = vec![0; lenn];

	println!("length is for cps {:?}", lenn);
	reader.read_exact(&mut buff)?;
	println!("buffer is for cps{:?}", buff);
	let mut counterparty_payment_script = ScriptBuf::from(buff);
	...
	match params {
		ReadUtilOpt::IsStub { keys, secp_ctx } => {
